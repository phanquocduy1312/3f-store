...
const accentMap: Record<string, { bg: string; text: string; ring: string; statsBg: string }> = {
  emerald: {
    ...
    statsBg: "bg-emerald-100/80 text-emerald-700",
  },
  amber: {
    bg: "bg-amber-50",
    text: "text-amber-600",
...
export function ReasonCard({ reason, index }: { reason: Reason; index: number }) {
  ...
  return (
    <MotionItem
      {...motionItemProps}
      className={`group relative flex flex-col overflow-hidden rounded-[2rem] bg-white p-8 shadow-[0_8px_30px_rgba(0,0,0,0.04)] ring-2 ring-transparent transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_25px_50px_rgba(16,133,79,0.1)] ${accent.ring}`}
    >
      ...
      <div className="relative z-10 flex flex-1 flex-col">
        <h3 className="mb-3 text-[1.75rem] font-black leading-tight tracking-tight text-[#0A2E1A] transition-colors duration-300 group-hover:text-forest">
          {reason.title}
        </h3>
        <p className="mt-auto text-base font-medium leading-[1.7] text-[#0A2E1A]/70">
          {reason.description}
        </p>
      </div>
    </MotionItem>
