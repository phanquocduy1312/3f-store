...
- Use `/preview --explain <topic>` skill to generate visual explanation with ASCII + Mermaid
- Use `/preview --diagram <topic>` skill for architecture and data flow diagrams
- Use `/preview --slides <topic>` skill for step-by-step walkthroughs
- Use `/preview --ascii <topic>` skill for terminal-friendly output only
- **HTML mode** (add `--html` for self-contained HTML pages, opens directly in browser):
  - `/preview --html --explain <topic>` skill — publication-quality HTML explanation
  - `/preview --html --diagram <topic>` skill — interactive HTML diagram with zoom controls
  - `/preview --html --slides <topic>` skill — magazine-quality slide deck
  - `/preview --html --diff [ref]` skill — visual diff review
  - `/preview --html --plan-review` skill — plan vs codebase comparison
  - `/preview --html --recap [timeframe]` skill — project context snapshot
